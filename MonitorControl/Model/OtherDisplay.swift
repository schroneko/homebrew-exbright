...
class OtherDisplay: Display {
  ...
  func restoreDDCSettingsToDisplay(command: Command) {
    if !self.smoothBrightnessRunning, !self.isSw(), !self.readPrefAsBool(key: .unavailableDDC, for: command), self.readPrefAsBool(key: .isTouched, for: command), prefs.integer(forKey: PrefKey.startupAction.rawValue) == StartupAction.write.rawValue, !app.safeMode {
      if command == .brightness, !prefs.bool(forKey: PrefKey.disableCombinedBrightness.rawValue) {
        os_log("Restoring combined brightness value %{public}@ for %{public}@", type: .info, String(self.readPrefAsFloat(for: .brightness)), self.name)
        _ = self.setDirectBrightness(self.readPrefAsFloat(for: .brightness))
        return
      }
  ...
  override func setDirectBrightness(_ to: Float, transient: Bool = false) -> Bool {
    ...
    if !self.isSw() {
      if !prefs.bool(forKey: PrefKey.disableCombinedBrightness.rawValue) {
        ...
        os_log("Combined brightness for %{public}@ target %{public}@ DDC %{public}@ SW %{public}@", type: .info, self.name, String(value), String(brightnessValue), String(brightnessSwValue))
  ...
  func asyncPerformWriteDDCValues(command: Command) {
    ...
    for controlCode in controlCodes {
      if Arm64DDC.isArm64 {
        if self.arm64ddc {
          didWrite = Arm64DDC.write(service: self.arm64avService, command: controlCode, value: value) || didWrite
        }
      } else {
        didWrite = (self.ddc?.write(command: controlCode, value: value, errorRecoveryWaitTime: 2000) ?? false) || didWrite
      }
    }
    var readback = "not-read"
    if let controlCode = controlCodes.first {
      Thread.sleep(forTimeInterval: 0.05)
      let values: (current: UInt16, max: UInt16)?
      if Arm64DDC.isArm64 {
        values = self.arm64ddc ? Arm64DDC.read(service: self.arm64avService, command: controlCode, numOfRetryAttemps: 3) : nil
      } else {
        values = self.ddc?.read(command: controlCode, tries: 3, minReplyDelay: nil)
      }
      if let values = values {
        readback = "\(values.current)/\(values.max)"
      }
    }
    os_log("DDC write result for %{public}@ command %{public}@ value %{public}@ success %{public}@ readback %{public}@", type: .info, self.name, String(reflecting: command), String(value), String(didWrite), readback)
  }
